...
def load_script():
    path = (
        ROOT
        / "scripts"
        / "n7_mixed_glynn_overlapping_24_rank_one_update_support_closure.py"
    )
    spec = importlib.util.spec_from_file_location("n7_overlap24_closure", path)
...
class Overlapping24RankOneUpdateSupportClosureTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        ...
        cls.payload = json.loads(
            (
                ROOT
                / "data"
                / "n7_mixed_glynn_overlapping_24_rank_one_update_support_closure.json"
    ...
    def test_exact_certificates_are_frozen(self):
        self.assertEqual(
            self.payload["status"],
            "EXACT_ALL_OVERLAPPING_24_42_PROJECTIVE_SUPPORT_CLOSURE",
        )
        self.assertEqual(
    ...
    def test_dense_internal_faces_are_complete(self):
        self.assertEqual(
            ...
            {
                "candidate_count": 900,
                "direct_primary_minor_rows": 325,
                "first_internal_face_rows": 527,
                "second_internal_face_rows": 48,
                "unresolved_rows": 0,
            },
        )

    def test_every_projective_coordinate_face_has_an_exact_family(self):
        self.assertEqual(self.payload["boundary_face_map"], self.module.FACE_MAP)
        ...
        for shape in ("extra_left", "extra_right"):
            ...
            self.assertEqual(len(face_map) - 1, 6)
