"""Exact local rank-six graph germ at the identity K3,2 matching."""

from __future__ import annotations

import argparse
import json
from pathlib import Path

import sympy as sp

try:
    from scripts.n6_product_32_rank_six_frame_barrier import beta, require
except ModuleNotFoundError:  # Direct script execution.
    from n6_product_32_rank_six_frame_barrier import beta, require


ROOT = Path(__file__).resolve().parents[1]
DEFAULT_JSON = ROOT / "data" / "n6_k32_graph_identity_germ.json"


def graph_pair(T: sp.Matrix) -> tuple[list[list[object]], list[list[object]]]:
    left: list[list[object]] = []
    right: list[list[object]] = []
    for row in range(3):
        for source_column in range(2):
            source = 2 * row + source_column
            vector_left: list[object] = [0] * 12
            vector_right: list[object] = [0] * 12
            vector_left[4 * row + source_column] = 1
            vector_right[4 * row + source_column] = 1
            for target in range(6):
                value = T[target, source]
                target_row, target_column = divmod(target, 2)
                vector_left[4 * target_row + target_column + 2] = value
                vector_right[4 * target_row + target_column + 2] = -value
            left.append(vector_left)
            right.append(vector_right)
    return left, right


def cross_matrix(T: sp.Matrix) -> sp.Matrix:
    left, right = graph_pair(T)
    return sp.Matrix([beta(x, y) for x in left for y in right])


def pivot_data(C0: sp.Matrix) -> tuple[list[int], list[int], list[int], list[int], sp.Matrix]:
    _, pivot_columns = C0.rref()
    _, pivot_rows = C0.T.rref()
    pivot_columns = list(pivot_columns)
    pivot_rows = list(pivot_rows)
    rows_out = [i for i in range(C0.rows) if i not in pivot_rows]
    columns_out = [j for j in range(C0.cols) if j not in pivot_columns]
    pivot = C0.extract(pivot_rows, pivot_columns)
    require(pivot.det() != 0, pivot.det())
    return pivot_rows, pivot_columns, rows_out, columns_out, pivot


def schur_derivative_certificate() -> dict[str, object]:
    identity = sp.eye(6)
    base = cross_matrix(identity)
    pivot_rows, pivot_columns, rows_out, columns_out, pivot = pivot_data(base)
    pivot_inverse = pivot.inv()
    base_pq = base.extract(pivot_rows, columns_out)
    base_rp = base.extract(rows_out, pivot_columns)

    derivative_columns: list[list[sp.Expr]] = []
    for target in range(6):
        for source in range(6):
            direction = sp.zeros(6)
            direction[target, source] = 1
            # Central difference is the exact first derivative because the
            # cross matrix is polynomial of degree at most two in T.
            derivative = (
                cross_matrix(identity + direction)
                - cross_matrix(identity - direction)
            ) / 2
            dpp = derivative.extract(pivot_rows, pivot_columns)
            dpq = derivative.extract(pivot_rows, columns_out)
            drp = derivative.extract(rows_out, pivot_columns)
            drq = derivative.extract(rows_out, columns_out)
            schur_derivative = (
                drq
                - drp * pivot_inverse * base_pq
                - base_rp * pivot_inverse * dpq
                + base_rp * pivot_inverse * dpp * pivot_inverse * base_pq
            )
            derivative_columns.append(list(schur_derivative))

    jacobian = sp.Matrix.hstack(
        *[sp.Matrix(column) for column in derivative_columns]
    )
    free_columns = [0, 7]  # T[0,0] and T[1,1].
    normal_columns = [i for i in range(36) if i not in free_columns]
    normal_jacobian = jacobian[:, normal_columns]
    _, minor_rows = normal_jacobian.T.rref()
    minor_rows = list(minor_rows)
    minor = normal_jacobian.extract(minor_rows, range(34))
    nullspace = jacobian.nullspace()
    require(base.rank() == 6, base.rank())
    require(jacobian.rank() == 34, jacobian.rank())
    require(normal_jacobian.rank() == 34, normal_jacobian.rank())
    require(minor.det() == -256, minor.det())
    require(len(nullspace) == 2, len(nullspace))
    expected_first = sp.zeros(36, 1)
    expected_second = sp.zeros(36, 1)
    for index in (0, 14, 28):
        expected_first[index] = 1
    for index in (7, 21, 35):
        expected_second[index] = 1
    nullspace_matrix = sp.Matrix.hstack(*nullspace)
    require(
        nullspace_matrix.columnspace() == sp.Matrix.hstack(
            expected_first, expected_second
        ).columnspace(),
        nullspace_matrix,
    )

    x, y = sp.symbols("x y")
    diagonal = sp.diag(x, y, x, y, x, y)
    diagonal_matrix = cross_matrix(diagonal)
    diagonal_pivot = diagonal_matrix.extract(pivot_rows, pivot_columns)
    diagonal_schur = (
        diagonal_matrix.extract(rows_out, columns_out)
        - diagonal_matrix.extract(rows_out, pivot_columns)
        * diagonal_pivot.inv()
        * diagonal_matrix.extract(pivot_rows, columns_out)
    )
    require(all(sp.simplify(entry) == 0 for entry in diagonal_schur), "diagonal Schur")
    require(sp.factor(diagonal_pivot.det()) == -8 * y**3, diagonal_pivot.det())

    return {
        "base_cross_matrix_shape": list(base.shape),
        "base_cross_rank": int(base.rank()),
        "pivot_rows": pivot_rows,
        "pivot_columns": pivot_columns,
        "pivot_determinant": int(pivot.det()),
        "schur_shape": [len(rows_out), len(columns_out)],
        "jacobian_shape": list(jacobian.shape),
        "jacobian_rank": int(jacobian.rank()),
        "tangent_dimension": 36 - int(jacobian.rank()),
        "free_coordinates": ["T[0,0]", "T[1,1]"],
        "normal_minor_rows": minor_rows,
        "normal_minor_determinant": int(minor.det()),
        "nullspace_generators": ["column_0_scaling", "column_1_scaling"],
        "diagonal_family": "diag(x,y,x,y,x,y)",
        "diagonal_pivot_determinant": "-8*y^3",
        "diagonal_schur_identically_zero": True,
    }


def diagonal_examples() -> dict[str, object]:
    rows = []
    for x, y in ((1, 1), (2, 3), (1, 2)):
        matrix = cross_matrix(sp.diag(x, y, x, y, x, y))
        rows.append({"x": x, "y": y, "cross_rank": int(matrix.rank())})
    require([row["cross_rank"] for row in rows] == [6, 6, 6], rows)
    return {"examples": rows}


def build_payload() -> dict[str, object]:
    return {
        "certificate": "N6-120",
        "status": "PURE_FORMAL_LOCAL_K32_GRAPH_GERM",
        "field": "characteristic zero",
        "hypothesis": (
            "L=graph(T), M=graph(-T) for T in GL_6 near identity, "
            "with the K3,2 3-by-4 rectangle map"
        ),
        "exact_certificate": schur_derivative_certificate(),
        "diagonal_examples": diagonal_examples(),
        "pure_consequence": (
            "The rank-at-most-six formal germ at T=I is exactly "
            "T=diag(x,y,x,y,x,y) in the pivot chart."
        ),
        "graph_component_consequence": (
            "The row-column torus orbit of T=I is this two-parameter family; "
            "inside the invertible graph incidence, the component through T=I "
            "is exactly that matching orbit."
        ),
        "boundary": [
            "does not classify arbitrary invertible T globally",
            "does not describe lower-rank collision points",
            "does not by itself supply the six-term Chow cocycle",
            "does not prove ordinary lower 29 or exact rank 32",
        ],
    }


def main() -> None:
    parser = argparse.ArgumentParser()
    parser.add_argument("--json", type=Path)
    parser.add_argument("--verify-json", type=Path)
    args = parser.parse_args()
    payload = build_payload()
    if args.json:
        args.json.write_text(json.dumps(payload, indent=2) + "\n", encoding="utf-8")
    if args.verify_json:
        require(
            payload == json.loads(args.verify_json.read_text(encoding="utf-8")),
            "frozen JSON differs from exact replay",
        )
    print("certificate=N6-120")
    print("base_rank=6")
    print("jacobian_rank=34")
    print("normal_minor=-256")
    print("status=PASS")


if __name__ == "__main__":
    main()
