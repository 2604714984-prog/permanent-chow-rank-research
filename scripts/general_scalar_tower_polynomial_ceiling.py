...
used by the proof.  It is regression evidence, not a substitute for the
uniform asymptotic argument.
"""
...
def exact_hypergeometric_diagnostics() -> dict[str, int]:
    ...
    require(pair_count == 1081, pair_count)
    require(cdf_count == 31579, cdf_count)
    return {
...
def geometric_start_diagnostics() -> int:
    ...
    for n in range(4, 201):
        ...
        central = comb(n, n // 2)
        # A uniform explicit central-binomial normalization sufficient for
        # comparing 2^n/sqrt(n) with the middle layer.
        require(2 * (n + 1) * central * central >= 4**n, (n, central))
    require(checks == 9900, checks)
    return checks
