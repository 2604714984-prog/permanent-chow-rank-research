...
def rectangle_union_minimum(
    ...
    Returns `(single_size, maximum_distinct_intersection, minimum_union)`.
    The finite loop checks all overlap-size pairs rather than all labelled
    rectangles.
    """
    ...
    for row_overlap in range(subset_size + 1):
        for column_overlap in range(subset_size + 1):
            if row_overlap == subset_size and column_overlap == subset_size:
                continue
            # Such overlaps occur only when two subsets of the given size can
            # have the displayed intersection in the ambient ground set.
...
def build_payload(maximum_m: int = 128) -> dict[str, Any]:
    ...
    # In the quartic boundary r_1+r_2>=16 and k<=2.  The larger component has
    # rank at least eight and hence at least six private polar directions.
