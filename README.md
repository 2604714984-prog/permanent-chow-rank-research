...
| `n=6` half-defect rank-32 candidate | conditional candidate; arithmetic replayed | the global image-span and filtration argument would prove `ChowRank(perm_6)=32` if the unrestricted half-defect quotient-symbol proposition is established; the current package does not independently derive its local rank rows |
