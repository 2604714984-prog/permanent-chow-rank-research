...
| `n=6` | post-audit repaired exact ordinary-rank proof; exact finite replay | `ChowRank(perm_6)=32`; the repaired half-defect proof uses actual five-variable derivative spaces, records and avoids the rejected formal-subproduct shortcut, and cancels every individual middle-rank defect; no border-rank claim |
| `n=7` | pure lower-bound theorem; exact recursive bivariate-shadow DPs; restricted endpoint certificates | `49<=ChowRank(perm_7)<=64`; the recursive shadow tower and a complementary raw catalectic prove the lower endpoint; synchronized mixed-Glynn rank-one projective support closures are exact through overlapping `(2,4)/(4,2)`, while its full `(3,4)` coupling defect is exactly 35, so that fixed packet is not on the Sylvester-equality locus; none of this proves lower 50, exact 64, or border rank |
...
| `n=6` biflag internal quadratic reduction | pure characteristic-zero core-chart theorem; complete coordinate enumeration; exact rational leakage replay; partial | the critical `72`-plane is intrinsically `E_2 intersect Sym^2(M)`; among all `1,352,078` coordinate twelve-planes only 34 product supports have intersection at least 15, every nonfactor first leakage has rank at least 6, and the full core-projection-isomorphism chart is product and excluded by N6-068; the remaining noncore biflag charts stay open |
| `n=6` biflag `3x4` graph charts | exact characteristic-zero graph certificate; pure product dimension gate; partial | on both coordinate `3x4` charts the effective linear quotient has minimum nonkernel leakage rank 6; the missing-core-column chart has 12 extra tail coordinates, but pure tail and corner-defect weights again have rank 6, so every plane with intersection at least 13 is a product; N6-068 excludes the full 15-dimensional actual-pair case; the four `4x3` endpoint charts remain |
| `n=6` critical biflag exclusion | exact characteristic-zero `4x3` graph certificates; pure projective-torus globalization | the three remaining `4x3` charts have kernels `18,30,28`, exactly product directions plus `12,24,24` truncated directions; pure truncated weights have rank 6 and vanish; all six fixed-point charts are therefore product at threshold 13, and projective component globalization puts the entire biflag high-intersection locus in the closed product locus |
| `n=6` partial `3x4` product-pair exclusion | characteristic-zero projective theorem; complete fixed-pair enumeration; exact rational/modular normal-cone certificate | complementary six-planes in `k^3 tensor k^4` have `E_34` cross-image dimension at least 6; this excludes cross-free product sections of dimensions 13, 14, and 15, closing the biflag `kappa2=1,2` layers (with `kappa2=3` closed by N6-107); both geometries at `kappa2=0` and the standard-hook `kappa2=1,2` layers remain open |
| `n=6` standard-hook partial-pair exclusion | characteristic-zero projective theorem; complete fixed-point and relative-normal replay; pure partial transverse rigidity | every standard-hook twelve-plane with quadratic intersection at least 13 is a `2x6`, `3x4`, or `4x3` product; N6-108 removes the latter two, while a thirteen-plane multiplier/ratio-algebra theorem turns every `2x6` edge into complete row slices; connectivity then contradicts hook dimension 23, excluding the standard-hook `kappa2=1,2` layers; only the two `kappa2=0` geometries remain at `a2=72` |
| `n=6` shadow-eleven actual-pair exclusion | pure characteristic-zero projective theorem; 7,200 fixed supports; exact rational linear/quadratic elimination; 432 symbolic branches | a twelve-dimensional permanent section difference between two six-planes always has twelve-dimensional shadow and transverse factors; at `a2=72,kappa2=0` all fifteen pair shadows are full, all factor planes are pairwise transverse, and their sum is the N6-101 23-plane; excluding the resulting standard-hook and biflag six-color configurations remains open |
| `n=6` twelve-dimensional `2x6` pair rigidity | pure characteristic-zero theorem; exact rational multiplier, invariant-locus tangent, and algebra replay | the N6-109 product threshold drops from 13 to 12; the only new ratio-algebra case is a reduced coordinate `3+3` equality whose generated algebra is the full 27-dimensional block parabolic, and its only complementary six-submodule pairs are complete row slices |
| `n=6` nonproduct twelve-shadow collision exclusion | exact projective theorem; complete `2*binom(23,12)` fixed-support enumeration; 9,624 full-rank pair Jacobians | in both N6-101 hooks every fixed point with quadratic intersection exactly 12 is absent or has a unique diagonal pair; all `432x72` pair-variable Jacobians have rank 72 already over `F_2`, so formal swap rigidity makes the full component diagonal; every surviving actual-pair component must specialize to one of 43 standard or 34 biflag product supports |
| `n=6` `3x4` rank-six fixed-stratum reduction | exact projective partial theorem; 853,776 fixed pairs; exact linear/quadratic weight certificates | the 2,424 rank-six fixed pairs form 20 orbits: 18 germs stay in a common eight-space, the `(4,1,1)` diagonal germ is swap-rigid, and the `(3,2,1)` staircase has a 20-generator squarefree quadratic initial obstruction whose weight semigroup cannot carry the complement determinant; any surviving complementary component must specialize further to a rank-three or rank-five fixed point |
| `n=6` `3x4` rank-five normal exclusion | pure characteristic-zero normal-cone theorem; exact bounded quadratic elimination and symbolic branch replay | all but three torus weight spaces contain every coefficient square in their exact minor span; the three survivors give two reduced points over each rank-five orbit, and their finite local models are a smooth diagonal curve or a node with ideal `(x1*x2)` whose branches all have sum rank at most ten; only the rank-three product fixed strata remain |
...
python scripts/n6_lower29_b34_first_shortening.py
python scripts/n6_product_shadow_b80_equality_locus.py
python scripts/n6_b34_x80_actual_endpoint_exclusion.py
python scripts/n6_product_shadow_b79_equality_locus.py
python scripts/n6_lower29_b34_x79_exclusion.py
python scripts/n6_product_shadow_b78_equality_locus.py
python scripts/n6_lower29_b34_x78_exclusion.py
python scripts/n6_product_shadow_b77_equality_locus.py
python scripts/n6_lower29_b34_x77_exclusion.py
python scripts/n6_product_shadow_b73_76_equality_locus.py
python scripts/n6_lower29_b34_x73_76_exclusion.py
python scripts/n6_product_shadow_b72_equality_locus.py
python scripts/n6_shadow11_pair_incidence_diagnostic.py --verify-json data/n6_shadow11_pair_incidence_exclusion.json
python scripts/n6_product_26_twelve_pair_rigidity.py --verify-json data/n6_product_26_twelve_pair_rigidity.json
python scripts/n6_lower29_b34_x72_frontier.py
python scripts/n6_b34_x72_one_relation_exclusion.py
...
python scripts/n6_product_34_partial_pair_exclusion.py
python scripts/n6_product_34_rank_six_fixed_reduction.py --verify-json data/n6_product_34_rank_six_fixed_reduction.json
python scripts/n6_product_34_rank_five_normal_exclusion.py --verify-json data/n6_product_34_rank_five_normal_exclusion.json
python scripts/n6_standard_hook_partial_pair_exclusion.py
```
