...
| N6-024 | `CONDITIONAL`, `COMPUTATION_REPLAYED` | if the unrestricted half-defect quotient-symbol estimate holds for every degree-six Chow term and every quotient of its actual factor span, then `ChowRank(perm_6)=32`; the finite rational rows and defect cancellation replay exactly, but the package does not independently derive the load-bearing local ranks | `docs/n6_exact_ordinary_chow_rank_32_candidate.md`, companion TeX/PDF, `scripts/n6_exact_ordinary_chow_rank_32_candidate.py`, and frozen JSON |
...
- N6-024 is a conditional proof candidate, not an exact unrestricted theorem. Its arithmetic replay accepts the half-defect rows as inputs; a complete independent derivation of those rows and their geometric scope remains required.
