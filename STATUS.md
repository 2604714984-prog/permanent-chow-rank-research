...
| S5-001 | `PROOF_DRAFT_COMPLETE`, `COMPUTATION_REPLAYED` | `ChowRank(perm_5)=16` over characteristic-zero fields | repaired v14 draft: closed projective incidence degeneration in characteristic zero plus an exact integral/modular endpoint certificate over 886,464 flags; fresh external review pending |
...
- External audit rejected v13 as a closed proof because its universal one-intersection implication and binary-cubic exclusion were not established. The v14 draft repairs those two gaps and replays the finite endpoint premise, but it has not yet received fresh independent external review or proof-assistant formalization.
...
- G-021 controls only the count-product separator and its canonical Fourier aggregate assignment. It does not lower-bound another aggregate assignment or the global two-defect family.
...
- The N6-022 sixteen-base aggregate assignment has exact cost 576. This closes that construction as a route to at most 25 terms but does not rule out another two-defect assignment with smaller actual support.
