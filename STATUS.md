...
| G-020 | `SUPERSEDED`, `PROOF_DRAFT_COMPLETE`, `COMPUTATION_REPLAYED`, `RESTRICTED_FAMILY_THEOREM` | the `2^(n-1)` fixed column-uniform Glynn sign products are linearly independent, and the unique expansion of `perm_n` in their span uses every term with nonzero coefficient | Walsh-Hadamard proof and `scripts/glynn_family_rigidity_audit.py`; subsumed by G-023 |
...
| G-023 | `PROOF_DRAFT_COMPLETE`, `COMPUTATION_REPLAYED`, `RESTRICTED_FAMILY_THEOREM` | over characteristic zero, `ColumnSignRank(perm_n)=RowSignRank(perm_n)=2^(n-1)`; the same lower bound holds with arbitrary off-diagonal coefficients when every row-zero anchor is nonzero and every normalized diagonal ratio is a sign | `docs/general_column_sign_rigidity.md`, the Boolean diagonal-slice/Walsh proof, primary audit through `n=10`, independent `n=6` replay, and compact frozen certificate |
| N6-001 | `OPEN` | improve the lower bound beyond 25 or find a shorter unrestricted decomposition of `perm_6` | `docs/n6_research_program.md` |
...
| N6-019 | `PROOF_DRAFT_COMPLETE`, `COMPUTATION_REPLAYED`, `RESTRICTED_FAMILY_THEOREM` | the 5,984-term normalized one-defect family has span dimension 987 and exact minimum support 32; the minimum now also follows from G-023 | `docs/n6_one_defect_sign_rigidity.md`, primary and independent audits, exact integer minors, frozen payload, and G-023 |
| N6-020 | `PROOF_DRAFT_COMPLETE`, `COMPUTATION_REPLAYED`, `ROUTE_DIAGNOSTIC` | the 467,264-term normalized two-defect family has exact parity-block ranks `406,406,406,322,322,207` and span dimension 11,533; an explicit quadratic separator uses only 24 base-labelled aggregate spaces, while G-023 proves the actual family minimum is 32 | `docs/n6_two_defect_sign_block_diagnostic.md`, exact rational elimination, frozen payload, and G-023 |
| N6-021 | `PROOF_DRAFT_COMPLETE`, `COMPUTATION_REPLAYED`, `RESTRICTED_AGGREGATE_THEOREM` | the N6-020 separator functions satisfy `rho_2(f)=rho_2(1-f)=46`; therefore the specific 24-base aggregate assignment has exact actual-term cost 744 | `docs/n6_two_defect_aggregate_atomic_rank.md`, exact local support classification, and frozen payload |
| N6-022 | `SUPERSEDED`, `PROOF_DRAFT_COMPLETE`, `COMPUTATION_REPLAYED`, `ROUTE_DIAGNOSTIC` | the count-product separator `g=n_4 n_5` gives an exact permanent representation in 16 nonzero base-labelled two-defect aggregate spaces; the former atomic-rank window `31<=rho_2(g)<=36` is superseded by N6-023 | `docs/n6_two_defect_sixteen_base_aggregate.md`, exact local support enumeration, and frozen payload |
| N6-023 | `PROOF_DRAFT_COMPLETE`, `COMPUTATION_REPLAYED`, `RESTRICTED_AGGREGATE_THEOREM` | `rho_2(n_4n_5)=36`, so the N6-022 sixteen-base assignment has exact cost 576; G-021 supplies a short general star/double-counting proof, while the original normal-form and meet-in-the-middle calculation remains an independent exhaustive certificate | `docs/general_two_defect_count_product_rank.md`, `docs/n6_two_defect_separator_rank36.md`, primary and independent G-021 audits, and the original N6-023 payload |
| N7-001 | `PROOF_DRAFT_COMPLETE`, `COMPUTATION_REPLAYED` | over an algebraically closed field of characteristic zero, `50<=ChowRank(perm_7)<=64` | `docs/n7_ordinary_chow_rank_lower50.md`; corrected degree-three/four endpoint proof; deterministic publication replays |
| N7-002 | `PROOF_DRAFT_COMPLETE`, `CONDITIONAL` | a 50-term decomposition is excluded when all summands have rank-seven factor planes forming a simple rank-seven 7-multilinear matroid | `docs/rethlas_perm7_20260822/round2_quartic_code/simple_n50_exclusion.md`; no coverage of rank-six mixtures, the no-direct-basis profile, or lengths 51 through 63 |
| N7-003 | `COMPUTATION_REPLAYED`, `ROUTE_DIAGNOSTIC` | the normalized Glynn packet is locally rigid in the audited tangent models, while scalar slope, one-step Tor/Koszul, and purely linear row-normal routes do not yield the unrestricted lower bound 64 | `docs/n7_rethlas_research_status_20260822.md` and the detailed Rethlas reports and replay programs |
| N7-004 | `OPEN` | prove `ChowRank(perm_7)=64` or find a decomposition with at most 63 terms | the sharp section route requires `borderCR(perm_7|x_77=0)>=63`; only the matching upper bound is currently available |
| C-001 | `CONJECTURE` | unrestricted `ChowRank(perm_n)=2^(n-1)` for all `n>=2` | Glynn upper bound; exact unrestricted results only for reviewed small `n` |

## Unverified items

- N7-001 has independent internal model audits and deterministic subclaim replays, but no named independent human review or proof-assistant formalization. The original exact-64 target remains open, the whole-problem verifier was not called, and no `blueprint_verified.md` exists.
- N7-002 and N7-003 are conditional or diagnostic results and must not be promoted to an unrestricted rank-64 theorem, global uniqueness of Glynn's decomposition, or a border-rank equality.
...
- G-023 is exact only for the column-sign family, its transpose, and the stated anchored diagonal-sign enlargement. It does not prove arbitrary complex row-homogeneous tensor rank or unrestricted Chow rank.
...
- N6-014 does not prove `ChowRank(perm_6)>=26`, a border Chow-rank lower bound of 25, or the conjectural unrestricted value 32.
- N6-015 through N6-017 are route diagnostics. They do not prove that lower 26 is impossible; they identify tested fixed-count, scalar-shadow, base-ratio, and scalar-homology routes that do not supply a strict global margin.
...
- G-020 is subsumed by G-023 but remains an independent fixed-subfamily Walsh replay.
- G-021 controls only the count-product separator and its canonical Fourier aggregate assignment. It does not lower-bound another unrestricted aggregate assignment.
- N6-019 retains its span-dimension and parity-structure content; its minimum-support conclusion is also a corollary of G-023.
- N6-020 determines linear block ranks and aggregate support. Its 24-base aggregate representation is not a 24-term decomposition; G-023 independently fixes the full two-defect minimum at 32.
- N6-021 and N6-023 prove exact costs only for their fixed aggregate assignments. Those costs are not lower bounds for arbitrary assignments, though G-023 now makes further sign-family optimization unnecessary.
- The sign-defect and full column-sign construction routes are closed. A shorter unrestricted decomposition, if one exists, must leave the sign and anchored diagonal-sign families.
