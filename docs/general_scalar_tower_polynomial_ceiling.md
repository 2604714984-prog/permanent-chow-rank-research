...
be the complete scalar derivative-tower lower bound defined in the preceding
...
degree, exact Ferrers product shadows, repeated block projection and direct
saturation, cannot by itself prove the conjectural Glynn-optimal lower bound
`2^(n-1)` for all large `n`.
...
## 1. Notation and previous exact inputs
...
and every Chow decomposition of `perm_n` has at least `Theta_n` terms.
...
The parent branch also proves the terminal localization estimate: if
`1<=K<=n/2`, then
...
The displayed version is deliberately one binomial layer weaker than the
sharp recorded statement, so floor and endpoint conventions do not matter in
the asymptotic argument below.

## 2. Two elementary threshold bounds

### Lemma 2.1 -- one-step saturation bound
...
For `t<Q_(n,d-1)`, the trivial bound `H_(n,d)(t)<=A_(n,d)` and

\[
(q-t)M_{n,d}\ge M_{n,d}^2=A_{n,d}
\]

show that the contribution in (1.3) is nonpositive.

For `t>=Q_(n,d-1)`, the preceding deficit is zero.  Hence
...
because `q>=M_(n,d)`.  Thus `D_(n,d)(q)=0`. ∎

### Corollary 2.2 -- a lower-half starting bound
...
The second inequality is the resulting geometric-series bound. ∎
...
The block-projection inequality gives
...
## 3. A central product-shadow smoothing lemma
...
Taking the conditional ratio gives (3.1), uniformly in `k`. ∎
...
`C_1/sqrt(n)`.  Since coordinate families are actual permanent-derivative
subspaces, this is a valid upper bound for the exact subspace minimum. ∎

## 4. Transporting saturation thresholds across the central window
...
Assume that both degree `d-1` and complementary shadow degree `n-d+1` lie in
the central range of Theorem 3.3.  Combining (1.2), (1.3), Theorem 3.3 and
Lemma 2.3 gives the following.
...
### Corollary 4.2 -- normalized lower-half transport
...
## 5. A parameterized scalar-tower ceiling
...
For large `n` the whole window and all complementary shadow degrees lie in
`[n/3,2n/3]`.
...
The sum of the binomial coefficients on the decreasing side is at most `2^n`.
Using the standard bound

\[
M_*\ge\frac{2^n}{\sqrt{2n}},
\]

summing (4.5) from the center to `d_+` costs only `O(M_*)`, plus a negligible
`O(w)` integrality term.  This proves (5.2). ∎
...
Divide by the central-binomial lower bound used above. ∎

Combining the two propositions yields the parameterized theorem
...
and the exponential tail in (5.6) is negligible.  Therefore
...
No further exact evaluation, larger dynamic program, extra derivative degree,
or repeated scalar block projection inside the same tower can establish
...
The smoothing lemma is an upper construction for arbitrary permanent
subspaces.  A Chow-realizable intersection may be much more rigid than an
arbitrary Ferrers extremizer.  Therefore the ceiling does not show that every
shadow-based idea is exhausted; it shows only that the current scalar tower,
which deliberately forgets Chow realizability after replacing it by a
dimension cap, is exhausted at this polynomial scale.

This objection is correct and identifies the next route.
...
The finite audits check the hypergeometric atom/CDF interface, the lower-half
geometric bound, central-binomial normalization and the exact PR #51 threshold
table.  They are regression evidence; the asymptotic proof is the argument
above.
