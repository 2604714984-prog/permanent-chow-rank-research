# Unit-weight packet-B common-graph coupling reduction

For a common 42-point graph packet whose 42 graph terms all have coefficient
one, let
...
More generally, if the graph-term coefficients form a nonzero diagonal matrix
`D`, the small condition is

\[
 \ker(E_4^T)\subseteq D\operatorname{im}(E_3).
\]

If `delta` is the defect of the relevant small inclusion, then the full packet
...
The exhaustive unit-coefficient monomial-curve box with six strictly increasing weights at
...
Thus coupling eliminates the complete unit-coefficient `(31,41)` branch in
this weight box. It does not eliminate that Hilbert profile once term
coefficients vary: over each test prime, choosing `D` from the unique
degree-four relation gives rank `BC=1225` and defect zero. The `(30,42)`
branch has no coupling obstruction because `E_4^T` is injective. All 130
point configurations were already independently rejected, for every choice
of nonzero term coefficients, by
the permanent degree-six and degree-seven target-containment tests, so no
decomposition survives this finite family.

This is a complete unit-coefficient coupling classification only for the
displayed common-graph monomial-curve box. It does not classify arbitrary
term coefficients, arbitrary 42-point sets, arbitrary
